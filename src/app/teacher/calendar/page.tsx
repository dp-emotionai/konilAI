...
import { createCalendarEvent, deleteCalendarEvent, getCalendarEvents, updateCalendarEvent, type CalendarEvent } from "@/lib/api/calendar";
...
export default function TeacherCalendarPage() {
  ...
  useEffect(() => {
    const client = new ChatClient((packet) => {
      ...
      if (type?.startsWith("calendar.event.")) void reloadEvents();
    });
  ...
  const handleEdit = useCallback(async () => {
    if (!editEventId || !editForm.title.trim() || !editForm.startsAt) return;

    await updateCalendarEvent(editEventId, {
      title: editForm.title.trim(),
      kind: editForm.kind?.trim() || "session",
      groupId: editForm.groupId.trim() ? editForm.groupId.trim() : null,
      sessionId: editForm.sessionId.trim() ? editForm.sessionId.trim() : null,
    ...
    setEditEventId(null);
    await reloadEvents();
  }, [editEventId, editForm, reloadEvents]);
      ...
      <Modal
        ...
        footer={
          <div className="flex items-center justify-end gap-2">
            ...
            <Button onClick={() => void handleEdit()} disabled={!editForm.title.trim() || !editForm.startsAt}>
              Сохранить
            </Button>
          </div>
        }
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <Input
              ...
              onChange={(e) => setEditForm((p) => ({ ...p, sessionId: e.target.value }))}
            />
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <div className="text-xs font-semibold text-slate-500">endsAt</div>
              <Input
                type="datetime-local"
