...
type StudentSessionPreparationModalProps = {
  open: boolean;
...
export default function StudentSessionPreparationModal({
                                                         open,
                                                         session,
                                                         onClose,
                                                         onStart,
  ...
  const refreshJoinInfo = async () => {
    const updated = await getSessionJoinInfo(session.id);
  ...
  const handleAcceptConsent = async () => {
    ...
    try {
      ...
      // Сохраняем глобальное согласие в backend, чтобы оно не терялось
      ...
      try {
        ...
        console.warn(
            "SESSION CONSENT AUDIT SAVE FAILED",
            sessionConsentError
        );
      }
      ...
      if (
          consentStatus.hasConsent &&
          updated?.reason !== "consent_required"
      ) {
        setStep(2);
      }
    } catch (error) {
  ...
  const handleRevokeConsent = async () => {
    ...
    try {
      ...
      console.error("SESSION CONSENT REVOKE FAILED", error);
      setConsentActionError(
          error instanceof Error
              ? error.message
        ...
        >
          <div className="flex min-h-full items-center justify-center">
            <section
                role="dialog"
                aria-modal="true"
                aria-labelledby="student-session-preparation-title"
                className={cn(
            ...
            >
              ...
              <div className="relative border-b border-slate-100 px-5 pb-5 pt-5 sm:px-7 sm:pb-6 sm:pt-6">
                <div className="flex items-center justify-between gap-4">
                  <div
                      id="student-session-preparation-title"
                    ...
                    <div className="grid items-center gap-7 md:grid-cols-[0.82fr_1.18fr] md:gap-10">
                      ...
                      <div>
                        ...
                        <div className="mt-3 text-sm font-medium leading-relaxed text-slate-500">
                          Видео не сохраняется. Анализ идёт 1–2 кадра в секунду, в
                          систему попадают только агрегированные метрики.
                        </div>

                        {blockReason === "consent_required" && (
                            <div className="mt-4 rounded-[18px] border border-purple-100 bg-purple-50/70 px-4 py-3.5">
                              <div className="text-xs font-bold text-[#7448FF]">
                                Требуется согласие
                              </div>
                              <div className="mt-1 text-sm font-semibold text-slate-800">
                                Для подключения к сессии нужно дать согласие на анализ
                                эмоций
                              </div>
                              <div className="mt-1 text-xs leading-relaxed text-slate-500">
                                Согласие обязательно по этике платформы. Его можно
                      ...
                      {sessionIsLive ? (
                          ...
                          <div className="rounded-[24px] border border-amber-200 bg-amber-50 px-6 py-8 text-center">
                            ...
                            <div className="mt-2 text-sm leading-relaxed text-amber-800">
                              Проверка камеры станет доступна, когда преподаватель
                              запустит сессию.
                            </div>
                          </div>
                      )}
              ...
              <div className="relative flex items-start justify-center gap-2.5 border-t border-slate-100 bg-slate-50/70 px-5 py-4 text-center sm:px-7">
                ...
                <div className="max-w-3xl text-[11px] font-medium leading-relaxed text-slate-500">
                  Соединение защищено по стандарту WebRTC P2P. Видео-поток не
                  записывается. Бэкенд получает только обезличенные числовые
                  метрики эмоций.
                </div>
              </div>
            </section>
