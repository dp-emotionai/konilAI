...
export default function StudentJoinSessionPage() {
  ...
  const [meetingView, setMeetingView] = useState<"speaker" | "gallery">("speaker");
            ...
            <div className="grid min-h-0 flex-1 grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(400px,460px)] xl:grid-cols-[minmax(0,1fr)_460px] 2xl:grid-cols-[minmax(0,1fr)_480px]">
              <div className="flex min-w-0 flex-col">
                <div className="shrink-0">
                  <div
                      ref={monitorRef}
                      className={cn(
                          "flex w-full flex-col overflow-hidden rounded-2xl bg-[#111111] shadow-[0_20px_70px_rgba(0,0,0,0.24)] ring-1 ring-black/10",
                          isMonitorFullscreen && "h-screen rounded-none"
                      )}
                  >
                    <div className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-[#1c1c1c] px-3 text-white sm:px-4">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="flex items-center gap-1.5 text-[11px] font-medium text-emerald-400">
                          <ShieldCheck size={15} />
                          <span className="hidden sm:inline">Защищённая встреча</span>
                        </div>
                        <span className="hidden h-4 w-px bg-white/15 sm:block" />
                        <span className="truncate text-xs font-medium text-white/80">{title}</span>
                      </div>
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        <button
                            type="button"
                            onClick={() =>
                                setMeetingView((view) => (view === "speaker" ? "gallery" : "speaker"))
                            }
                        ...
                        >
                          <Layout size={15} />
                          {meetingView === "speaker" ? "Галерея" : "Докладчик"}
                        </button>
                        <button
                            type="button"
                            ...
                            aria-label={isMonitorFullscreen ? "Выйти из полного экрана" : "Открыть полный экран"}
                        >
                          <Maximize2 size={15} />
                        </button>
                      </div>
                    ...
                    >
                      {meetingView === "gallery" ? (
                          <div className="grid h-full grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
                            <ZoomVideoTile
                                stream={remoteStream}
                                name={formatParticipantLabel(teacherParticipant)}
                            />
                            <ZoomVideoTile
                          ...
                          <div className="relative h-full">
                            ...
                            <div className="absolute bottom-3 right-3 z-20 h-24 w-36 sm:h-32 sm:w-52 xl:h-40 xl:w-64">
                              <ZoomVideoTile
                                  stream={localMeetingStream}
                                  name={currentStudentName}
                                  isLocal
                      ...
                      <div className="pointer-events-none absolute left-4 top-4 z-20 flex flex-wrap gap-2">
                        {connectionState !== "connected" && (
                            <div className="flex items-center gap-1.5 rounded-md bg-amber-500 px-2.5 py-1 text-[10px] font-semibold text-white shadow-lg">
                              <AlertCircle size={12} />
                              {connectionState === "connecting" ? "Подключение" : "Нет соединения"}
                            </div>
                        )}
                        {wsDisconnected && (
                            <div className="flex items-center gap-1.5 rounded-md bg-red-600 px-2.5 py-1 text-[10px] font-semibold text-white shadow-lg">
                    ...
                    <ZoomMeetingControls
                        ...
                        onToggleParticipants={() =>
                            document.getElementById("student-session-participants")?.scrollIntoView({
                              behavior: "smooth",
                              block: "start",
                            })
                        }
                        onToggleFullscreen={() => void toggleMonitorFullscreen()}
                        onMore={() =>
                            document.getElementById("student-session-tools")?.scrollIntoView({
                              behavior: "smooth",
                              block: "start",
                            })
                        }
                ...
                <div id="student-session-tools" className="min-w-0 flex-1 space-y-6 pt-6">
                  <div>
              ...
              >
                ...
                <div className="bg-white border-slate-100 border rounded-[28px] shadow-[0_4px_24px_rgba(0,0,0,0.02)] p-6 space-y-6 shrink-0">
                  ...
                  <div id="student-session-participants" className="space-y-4">
                    <div>
