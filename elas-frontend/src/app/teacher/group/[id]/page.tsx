...
export default function TeacherGroupDetailPage() {
  ...
  const [invitableStudents, setInvitableStudents] = useState<InvitableStudentRow[]>([]);
        ...
        .then((data) => {
          ...
          setSelectedInviteIds((prev) =>
              prev.filter((studentId) =>
                  data.some((student) => student.id === studentId && !student.alreadyInvited)
              )
          );
        })
        .catch(() => {
  ...
  const selectedInviteStudents = useMemo(
      () =>
          invitableStudents.filter(
              (student) => selectedInviteIds.includes(student.id) && !student.alreadyInvited
          ),
  ...
  const handleInviteSubmit = async () => {
    ...
    try {
      ...
      setInviteSuccess(n > 0 ? `Отправлено приглашений: ${n}` : "Выбранные студенты уже приглашены.");
      setSelectedInviteIds([]);
        ...
        <Modal
            ...
            footer={
              <div className="flex justify-end gap-2">
                ...
                <Button disabled={inviteLoading || selectedInviteStudents.length === 0} onClick={handleInviteSubmit}>
                  {inviteLoading ? "Отправка…" : `Отправить (${selectedInviteStudents.length})`}
                </Button>
              </div>
            }
        >
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between text-xs text-muted">
              <span>Доступно студентов: {invitableStudents.filter((student) => !student.alreadyInvited).length}</span>
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                disabled && "cursor-not-allowed opacity-55 hover:bg-surface"
                            )}
                        >
                    <span
                        className={cn(
                    ...
                    >
                      {selected && <CheckCircle2 size={14} />}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-0.5 truncate text-xs text-muted">{student.email}</div>
                          </div>
